...
def seed_demo_data(store: MailStore) -> None:
    ...
    store.add_message(
        ...
        subject="Welcome to mcpMail",
        sender="demo@example.com",
        recipients="you@example.com",
        received_at="2026-01-03T12:00:00",
        body_preview="This is local demo data from SQLite.",
    )
    store.add_message(
        account.id,
        inbox.id,
        message_id="<local-cache@example.com>",
        subject="Local cache connected to the GUI",
        sender="demo@example.com",
        recipients="you@example.com",
        received_at="2026-01-02T12:00:00",
        is_read=True,
        body_preview="The window now displays data from MailStore.",
    )
    store.add_message(
        account.id,
        sent.id,
        message_id="<sent-demo@example.com>",
        subject="Sent demo message",
        sender="you@example.com",
        recipients="demo@example.com",
        sent_at="2026-01-01T12:00:00",
        body_preview="This is only local demo data.",
    )
