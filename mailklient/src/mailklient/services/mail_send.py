...
class MailSendService:
    ...
    def send_draft(self, draft: ComposeDraft) -> SendResult:
        ...
        if not recipients:
            raise ValueError("Legg inn minst en mottaker.")

        sent_at = datetime.now(timezone.utc)
        message_id = make_msgid(domain=_message_id_domain(settings.email_address))
        sent = self._smtp_client_class(settings.smtp).send_message(
        ...
        if sent:
            self._save_sent_copy(draft, settings.email_address, message_id, sent_at)
